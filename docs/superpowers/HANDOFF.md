...
**State (2026-07-19, tag `m4-on-foot`):** Milestones 1–4 complete. M4: third-person blocky character (walk/sprint/jump, procedural walk swing), lazy trimesh collision ring (280 m around active camera; water/props/landmarks excluded; buildings need concave shapes — NOT watertight), street+district HUD (64 m name grid + district boxes in metadata, 2 MB streets list gone), spawn from metadata (consumed), F toggles debug fly-cam, mouse capture owned by main.gd, camera scripts gated on `current`. Smoke now prints PLAYER OK / LANDED OK y=0.0 / SMOKE OK: 314 tiles. Screenshots `docs/screenshots/m4_onfoot_*.png`. ENV NOTES: fresh clone must run `--import` once before headless scripts using class_name types; headless frame:physics ≈ 2:1 — never fixed-frame physics waits in tests, poll conditions. Next: M5 "Behind the wheel" (drivable car — Godot Easy Vehicle Physics MIT + Kenney Car Kit CC0 need asset downloads, or procedural blocky car same as character; enter/exit prompt, camera swap, speedometer). Open carry-forwards: landmark collision missing, HUD has no unit test, fly-cam Esc manual test, terrain loop perf, minimap.gd guards.
